...
ts <- lm(Deaths ~ Year + I + P, data=HOMICIDE_STATES_SYG)
...
NO_SYG_Var<- c("Year","State","Deaths","I","P")
NO_SYG<-HOMICIDE_STATES_NOSYG[NO_SYG_Var]
NO_SYG["P"]=0

plot(NO_SYG$Year, NO_SYG$Deaths,
      bty="n", pch=19, col="gray",
      ylim = c(min(NO_SYG$Deaths, na.rm = TRUE), max(NO_SYG$Deaths, na.rm = TRUE)), xlim=c(1999,2020),
      xlab = "Time (years)", 
      ylab = "Homicide Deaths by Firearms")

#find regression of No SYG states
ts_NOSYG <- lm(Deaths ~ Year, data=NO_SYG)
lines(NO_SYG$Year, ts_NOSYG$fitted.values, col="steelblue", lwd=2)
summary(ts_NOSYG)

regTS_NOSYG = lm(Deaths ~ Year, data=NO_SYG)# Our time series model
...
```{r}
#Plot all States 
plot(HOMICIDE_States$Year, HOMICIDE_States$Deaths,
      bty="n", pch=19, col="gray",
      ylim = c(min(HOMICIDE_States$Deaths, na.rm = TRUE), max(HOMICIDE_States$Deaths, na.rm = TRUE)), xlim=c(1999,2020),
      xlab = "Time (years)", 
      ylab = "Homicide Deaths by Firearms")
